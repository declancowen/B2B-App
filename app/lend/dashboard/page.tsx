...
export default function LenderBidDashboardPage() {
  ...
  const dummyCompanies = [
    ...
    { id: 3, name: "Manufacturing Plus", loanValue: "R3,200,000", loanTerm: "24 months", loanType: "Bridging Finance", dueDate: "2024-01-20", daysUntilDue: 12, status: "Cancelled" },
    { id: 4, name: "Retail Dynamics", loanValue: "R1,800,000", loanTerm: "6 months", loanType: "Trade Finance", dueDate: "2024-01-12", daysUntilDue: 4, status: "Rejected" },
    ...
    { id: 6, name: "Digital Marketing Inc", loanValue: "R950,000", loanTerm: "9 months", loanType: "Trade Finance", dueDate: "2024-01-18", daysUntilDue: 10, status: "Expired" },
    ...
    { id: 11, name: "Media Solutions Ltd", loanValue: "R2,500,000", loanTerm: "18 months", loanType: "Trade Finance", dueDate: "2024-01-08", daysUntilDue: -2, status: "Cancelled" }
  ]
  ...
  const handleCancel = (companyId: number) => {
    console.log('Cancel company:', companyId)
  ...
  const getFilteredCompanies = () => {
    if (activeTab === 'all') {
      ...
      return dummyCompanies.filter(company => company.status === 'New') // Only companies with "New" status
      ...
      return dummyCompanies.filter(company => company.id === 3 || company.id === 4 || company.id === 6) // Manufacturing Plus (Cancelled), Retail Dynamics (Rejected), Digital Marketing Inc (Expired)
    }
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {getFilteredCompanies().map((company) => (
            ...
            >
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-md border shadow-sm ${getStatusBadgeStyle(
                  // Override TechCorp status based on tab
                  activeTab === 'opportunities' && company.id === 1 ? 'Signed' :
                  activeTab === 'all' && company.id === 1 ? 'New' : 
                  company.status
                )}`}>
                  {activeTab === 'opportunities' && company.id === 1 ? 'Signed' :
                   activeTab === 'all' && company.id === 1 ? 'New' : 
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="flex space-x-2">
                  {/* All companies have a view button */}
                  <button
                    onClick={() => handleView(company.id)}
                  ...
                  {/* Delete button for "New" status only */}
                  {(company.status === 'New' || (activeTab === 'all' && company.id === 1)) && (
                    <button
                  ...
                  {/* Cancel button for everything else except Cancelled, Rejected, Expired, and Signed */}
                  {company.status !== 'New' && 
                   company.status !== 'Cancelled' && 
                   company.status !== 'Rejected' && 
                   company.status !== 'Expired' && 
                   !(activeTab === 'opportunities' && company.id === 1) && // Don't show cancel for TechCorp in opportunities (Signed)
                   !(activeTab === 'all' && company.id === 1) && ( // Don't show cancel for TechCorp in all tab (New)
                    <button
                      onClick={() => handleCancel(company.id)}
                      className="bg-orange-600 text-white p-2 rounded-md hover:bg-orange-700 transition-colors duration-200"
                      title="Cancel"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M18 6 6 18"/>
